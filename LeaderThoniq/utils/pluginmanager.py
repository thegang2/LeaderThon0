...
from leaderthon import CMD_HELP, LOAD_PLUG
...
LOGS = logging.getLogger("leaderthon")
...
def load_module(shortname, plugin_path=None):
    if shortname.startswith("__"):
        ...
        path = Path(f"leaderthon/plugins/{shortname}.py")
        checkplugins(path)
        name = "leaderthon.plugins.{}".format(shortname)
        ...
        if plugin_path is None:
            path = Path(f"leaderthon/plugins/{shortname}.py")
            name = f"leaderthon.plugins.{shortname}"
        else:
        ...
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["leaderthon.plugins." + shortname] = mod
        LOGS.info("᯽︙تم بنجاح تحميل ملف ✓" + shortname)
...
def start_assistant(shortname):
    if shortname.startswith("__"):
        ...
        path = Path(f"leaderthon/plugins/assistant/{shortname}.py")
        name = "leaderthon.plugins.assistant.{}".format(shortname)
        spec = importlib.util.spec_from_file_location(name, path)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)
        ...
        mod.tgbot = bot.tgbot
        spec.loader.exec_module(mod)
        sys.modules["leaderthon.plugins.assistant" + shortname] = mod
