...
from ALKaido import get_string
...
from leaderthon import BOTLOG_CHATID
from ..Config import Config
from ..helpers.utils import _format
from ..sql_helper.globals import addgvar, delgvar, gvarstatus
from . import AUTONAME, DEFAULT_GROUP, DEFAULT_BIO, edit_delete, leaderthoniq, logging
...
autopic_path = os.path.join(os.getcwd(), "leaderthon", "original_pic.png")
digitalpic_path = os.path.join(os.getcwd(), "leaderthon", "digital_pic.png")
digital_group_pic_path = os.path.join(os.getcwd(), "leaderthon", "digital_group_pic.png")
autophoto_path = os.path.join(os.getcwd(), "leaderthon", "photo_pfp.png")
auto_group_photo_path = os.path.join(os.getcwd(), "leaderthon", "photo_pfp.png")
...
async def digitalpicloop():
    ...
    while DIGITALPICSTART:
        ...
        jep = gvarstatus("DEFAULT_PIC") or "leaderthon/helpers/styles/PaybAck.ttf"
        fnt = ImageFont.truetype(jep, 65)
        drawn_text.text((200, 200), current_time, font=fnt, fill=colo)
        img.save(autophoto_path)
        file = await leaderthoniq.upload_file(autophoto_path)
        try:
            if i > 0:
                await leaderthoniq(
                    functions.photos.DeletePhotosRequest(
                        await leaderthoniq.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            await leaderthoniq(functions.photos.UploadProfilePhotoRequest(file))
...
async def autoname_loop():
    ...
    while AUTONAMESTART:
        ...
        LOGS.info(name)
        try:
            await leaderthoniq(functions.account.UpdateProfileRequest(last_name=name))
...
async def autobio_loop():
    ...
    while AUTOBIOSTART:
        ...
        LOGS.info(bio)
        try:
            await leaderthoniq(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
        await asyncio.sleep(Config.CHANGE_TIME)
        AUTOBIOSTART = gvarstatus("autobio") == "true"
...
@leaderthoniq.on(admin_cmd(pattern=f"{phow8t}(?:\s|$)([\s\S]*)"))
async def _(event):
...
@leaderthoniq.on(admin_cmd(pattern=f"{namew8t}(?:\s|$)([\s\S]*)"))
async def _(event):
...
@leaderthoniq.on(admin_cmd(pattern=f"{biow8t}(?:\s|$)([\s\S]*)"))
async def _(event):
...
leaderthoniq.loop.create_task(digitalpicloop())
leaderthoniq.loop.create_task(autoname_loop())
leaderthoniq.loop.create_task(autobio_loop())
