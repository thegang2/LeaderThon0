...
from leaderthon.core.logger import logging
...
#ترجمه وكتابة فريق لـيـدرثـون
@leaderthoniq.ar_cmd(incoming=True, func=lambda e: e.is_private, edited=False, forword=None)
...
@leaderthoniq.ar_cmd(outgoing=True, func=lambda e: e.is_private, edited=False, forword=None)
...
#ترجمه وكتابة فريق لـيـدرثـون
@leaderthoniq.tgbot.on(CallbackQuery(data=re.compile(rb"show_pmpermit_options")))
...
#ترجمه وكتابة فريق لـيـدرثـون
@leaderthoniq.tgbot.on(CallbackQuery(data=re.compile(rb"to_enquire_something")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
...
@leaderthoniq.tgbot.on(CallbackQuery(data=re.compile(rb"to_request_something")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "⌯︙- عذرا هذه الخيارات ليست لك انها للمستخدمين الذين يراسلوك 🧸♥"
        return await event.answer(text, cache_time=0, alert=True)
...
#ترجمه وكتابة فريق لـيـدرثـون
@leaderthoniq.tgbot.on(CallbackQuery(data=re.compile(rb"to_chat_with_my_master")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "⌯︙- عذرا هذه الخيارات ليست لك انها للمستخدمين الذين يراسلوك 🧸♥"
        return await event.answer(text, cache_time=0, alert=True)
...
@leaderthoniq.tgbot.on(CallbackQuery(data=re.compile(rb"to_spam_my_master_inbox")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = " عذرا هذه الخيارات ليست لك انها للمستخدمين الذين يراسلوك 🧸♥"
        return await event.answer(text, cache_time=0, alert=True)
    ...
    try:
        ...
        PM_WARNS = {}#ترجمه وكتابة فريق لـيـدرثـون
    if str(event.query.user_id) in PM_WARNS:
        del PM_WARNS[str(event.query.user_id)]
        sql.del_collection("pmwarns")
        sql.add_collection("pmwarns", PM_WARNS, {})
...
async def pmpermit_on(event):
    ...
    input_str = event.pattern_match.group(1)
    if input_str == "تعطيل":
        if gvarstatus("pmmenu") is None: #ترجمه وكتابة فريق لـيـدرثـون 
...
async def approve_p_m(event):  # sourcery no-metrics
    ...
    if not reason:
        reason = "لـم يـذكر"
    try: #ترجمه وكتابة فريق لـيـدرثـون
        PM_WARNS = sql.get_collection("pmwarns").json
    except AttributeError:
        PM_WARNS = {}
...
async def approve_p_m(event):
    ...
    await edit_or_reply(
        event,
        APPROVED_PMs,
        file_name="قائـمة الحـماية لـيـدرثـون.txt",
        caption="قائـمة ا@z_r_Hlلـمسموح لـهم الـحالية\n سـورس لـيـدرثـون الـعربي \n @leaderthon",
    )#ترجمه وكتابة فريق لـيـدرثـون
