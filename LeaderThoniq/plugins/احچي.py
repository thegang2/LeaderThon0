...
@leaderthoniq.ar_cmd(pattern="احجي(?:\s|$)([\s\S]*)",
...
@leaderthoniq.ar_cmd(pattern="استخرج(?:\s|$)([\s\S]*)",
...
async def _(event):
    ...
    pic_file = await leaderthoniq.download_media(reply, Config.TMP_DOWNLOAD_DIRECTORY)
    if not pic_file:
