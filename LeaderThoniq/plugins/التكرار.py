...
async def spam_function(event, leaderthon, leaderthoniq, sleeptimem, sleeptimet, DelaySpam=False):

    counter = int(leaderthoniq[0])
    if len(leaderthoniq) == 2:
        spam_message = str(leaderthoniq[1])
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            if event.reply_to_msg_id:
                await leaderthon.reply(spam_message)
            else:
                await event.client.send_message(event.chat_id, spam_message)
            await asyncio.sleep(sleeptimet)
    elif event.reply_to_msg_id and leaderthon.media:
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            leaderthon = await event.client.send_file(
                event.chat_id, leaderthon, caption=leaderthon.text
            )
            await _catutils.unsavegif(event, leaderthon)
        ...
        if BOTLOG:
            ...
            leaderthon = await event.client.send_file(BOTLOG_CHATID, leaderthon)
            await _catutils.unsavegif(event, leaderthon)
        return
    elif event.reply_to_msg_id and leaderthon.text:
        spam_message = leaderthon.text
        for _ in range(counter):
            if gvarstatus("spamwork") is None:
                return
            await event.client.send_message(event.chat_id, spam_message)
...
@leaderthoniq.ar_cmd(pattern="كرر (.*)")
async def spammer(event):
    leaderthon = await event.get_reply_message()
    leaderthoniq = ("".join(event.text.split(maxsplit=1)[1:])).split(" ", 1)
    try:
        counter = int(leaderthoniq[0])
    except Exception:
    ...
    await spam_function(event, leaderthon, leaderthoniq, sleeptimem, sleeptimet)

@leaderthoniq.on(admin_cmd(pattern="(مكرر|مؤقت|موقت)"))
async def spammer(event):
    ...
    leaderthoniq = input_str[1:]
    await event.delete()
    addgvar("spamwork", True)
    await spam_function(event, reply, leaderthoniq, sleeptimem, sleeptimet, DelaySpam=True)


@leaderthoniq.ar_cmd(pattern="تكرار الملصق$")
...
@leaderthoniq.ar_cmd(pattern="سبام (.*)")
...
@leaderthoniq.ar_cmd(pattern="وسبام (.*)")
async def tmeme(event):
...
@leaderthoniq.ar_cmd(pattern="ايقاف التكرار ?(.*)")
