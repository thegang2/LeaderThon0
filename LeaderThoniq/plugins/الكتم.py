...
@leaderthoniq.on(admin_cmd(pattern=f"كتم(?:\s|$)([\s\S]*)"))
async def mutejep(event):
    if event.is_private:
        ...
        if event.chat_id == leaderthoniq.uid:
            return await edit_delete(event, "**𖡛... . لمـاذا تࢪيـد كتم نفسـك؟  ...𖡛**")
        if event.chat_id == 6561639358:
...
@leaderthoniq.on(admin_cmd(pattern=f"الغاء كتم(?:\s|$)([\s\S]*)"))
