from leaderthoniq import leaderthoniq
...
@leaderthoniq.ar_cmd(pattern="المكاتب")
...
@leaderthoniq.ar_cmd(
    ...
    info={
        "header": "To list all plugins in leaderthoniq.",
        "usage": "{tr}plugins",
    },
)
async def _(event):
    "To list all plugins in leaderthoniq"
    cmd = "ls leaderthoniq/plugins"
    o = (await _catutils.runcmd(cmd))[0]
    OUTPUT = f"**[ليدرثون](tg://need_update_for_some_feature/) الـمـلفـات:**\n{o}"
...
@leaderthoniq.ar_cmd(
    ...
    info={
        "header": "To list all environment values in leaderthoniq.",
        "description": "to show all heroku vars/Config values in your leaderthoniq",
        "usage": "{tr}env",
    },
)
async def _(event):
    "To show all config values in leaderthoniq"
    cmd = "env"
    o = (await _catutils.runcmd(cmd))[0]
    OUTPUT = (
        f"**[ليدرثون](tg://need_update_for_some_feature/) قـائمـة الـفـارات:**\n\n\n{o}\n\n**انتبه هنالك معلومات حساسة لا تُعطِها لشخص غير موثوق**"
    )
    ...
    await leaderthoniq.send_message("me", OUTPUT)
...
@leaderthoniq.ar_cmd(pattern="رابط مباشر")
