#    جميع الحقوق محفوظة كتابة وتعديل  :   @XxKaido
                                 ...
                                 ChatAdminRights,
                                   ChatBannedRights,
                                     MessageEntityMentionName,
                                       MessageMediaPhoto)
from leaderthon.utils import admin_cmd
from ..Config import Config
from leaderthon import CMD_HELP, leaderthoniq
...
async def get_full_user(event):  
    ...
    if event.reply_to_msg_id and not len(args) == 2:
        ...
        try:
            ...
            return await event.edit("▾∮ هنالك خطأ يرجى تبليغنا @leaderthon", str(err))           
...
async def get_user_from_id(user, event):
    ...
    try:
        ...
        return None
    return user_obj
@leaderthoniq.on(admin_cmd(pattern="{up_admin} ?(.*)"))
async def gben(leaderthon):
    dc = razan = leaderthon
    i = 0
    sender = await dc.get_sender()
    me = await leaderthon.client.get_me()
    await razan.edit("▾∮ يتم رفع المستخدم في جميع المجموعات")
    my_mention = "[{}](tg://user?id={})".format(me.first_name, me.id)
    f"@{me.username}" if me.username else my_mention
    await leaderthon.get_chat()
    if leaderthon.is_private:
        user = leaderthon.chat
        rank = leaderthon.pattern_match.group(1)
    else:
        leaderthon.chat.title
    try:
        user, rank = await get_full_user(leaderthon)
    except:
        pass
    if me == user:
       leaderthoniq = await razan.edit("▾∮ لا استطيع رفع نفسي 🧸🤍،")
       return
    try:
        if not rank:
            rank = "ㅤㅤ"
    ...
    if user:
        telchanel = [d.entity.id
                     for d in await leaderthon.client.get_dialogs()
        ...
        for x in telchanel:
          try:
             await leaderthon.client(EditAdminRequest(x, user, rgt, rank))
             i += 1
...
@leaderthoniq.on(admin_cmd(pattern="{down_admin} ?(.*)"))
async def gben(leaderthon):
    dc = razan = leaderthon
    i = 0
    sender = await dc.get_sender()
    ...
    try:
        ...
        pass
    if me == user:
       leaderthoniq = await razan.edit("▾∮ لا استطيع تنزيل نفسي 🧸🤍")
       return
    try:
        if not rank:
            rank = "ㅤㅤ"
