# Copyright (C) 2021 leaderthon TEAM
# FILES WRITTEN BY  @XxKaido
import html
import base64
...
DEFAULTUSERBIO = (
    str(DEFAULT_BIO)
    if DEFAULT_BIO
    else "الحمد لله دائماً وابداً 🎀 𝑆𝐻 : @leaderthon"
)


@leaderthoniq.ar_cmd(pattern="انتحال(?:\s|$)([\s\S]*)")
async def _(event):
    mid = await leaderthoniq.get_me()
...
#Kaido
jeps = ["leaderthon", "leaderthonsupport"]
@leaderthoniq.ar_cmd(pattern="انتحال_الدردشه")
async def Kaido(event):
    if event.is_group or event.is_channel:
        ...
        chat_id = msg
        try:
            result = await leaderthoniq(GetFullChannelRequest(
                chat_id
            ))
        except ValueError:
        ...
        mych = await leaderthoniq(GetFullChannelRequest(
                event.chat_id
            ))
        if msg in jeps:
        ...
        try:
            await leaderthoniq(functions.channels.EditTitleRequest(
                channel=await leaderthoniq.get_entity(event.chat_id),
        ...
        try:
            await leaderthoniq(functions.messages.EditChatAboutRequest(
            peer=event.chat_id,
            about=result.full_chat.about
        ))
        except FloodWaitError:
            return await edit_delete(event, "**انتضر مدة لا تقل عن 5 دقائق للانتحال مجدداً FLOODWAITERROR خطأ من التيليجرام**")
        profile_pic = await leaderthoniq.download_profile_photo(chat_id, Config.TEMP_DIR)
        pfile = await leaderthoniq.upload_file(profile_pic)
        try:
            await leaderthoniq(functions.channels.EditPhotoRequest(event.chat_id, pfile))
        except FloodWaitError:
            return await edit_delete(event, "**انتضر مدة لا تقل عن 5 دقائق للانتحال مجدداً FLOODWAITERROR خطأ من التيليجرام**")
        ...
        if len(messageo) != 8:
            return await edit_delete(event, "لا تغير الرسالة @leaderthon")
        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID,
...
#Kaido
@leaderthoniq.ar_cmd(pattern="اعادة_الدردشه")
async def Kaido_back(event):
    if event.is_group or event.is_channel:
        if gvarstatus (f"{event.chat_id}name"):
            ...
            async for photo in leaderthoniq.iter_profile_photos(event.chat_id, limit=1) :
                    await leaderthoniq(
