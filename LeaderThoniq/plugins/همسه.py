...
async def wspr(event):
    if event.fwd_from:
        return
    leaderthoniqb = event.pattern_match.group(1)
    rrrd7 = "@nnbbot"
    if event.reply_to_msg_id:
        reply_to_id = await event.get_reply_message()
    tap = await bot.inline_query(rrrd7, leaderthoniqb) 
...
# كتابة وتعديل فريق الجوكر  #@XxKaidol
