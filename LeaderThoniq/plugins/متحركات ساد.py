...
#جميع الحقوق محفوظة لسـورس ليدرثون تخـمط تبيـن فشلـك
...
async def tmgif(event):
    if event.fwd_from:
        return
    XxKaido = await reply_id(event)
    if sad:
        leaderthoniq = f"**˛ JEP ، ٰ𝖦𝗂𝖿 𝖲ِ𝖺ٰ𝖣 . .**\n"
        leaderthoniq += f"✛━━━━━━━━━━━━━✛\n"
        leaderthoniq += f"**الـمتحـرڪـة الأولـى **"
        await event.client.send_file(event.chat_id, sad, caption=leaderthoniq, reply_to=XxKaido)
