...
@leaderthoniq.ar_cmd(pattern="tdata")

async def td(event):
    return await edit_or_reply(event, str(t))

@leaderthoniq.ar_cmd(pattern="توب الفلوس(.*)")
...
@leaderthoniq.ar_cmd(pattern="مسح حسابي(.*)")
...
@leaderthoniq.on(admin_cmd(pattern="(فلوسي|اموالي) ?(.*)"))
async def a(message):
    me = await message.client.get_me()
...
@leaderthoniq.on(admin_cmd(pattern="(بنكي|مصرفي) ?(.*)"))
...
@leaderthoniq.ar_cmd(func=lambda m:"راتب")
...
@leaderthoniq.ar_cmd(pattern="انشاء حساب (.*)")
