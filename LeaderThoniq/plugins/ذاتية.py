...
import os
from leaderthon import *
#شعندك داخل هنا منيوك 😂
@leaderthoniq.on(admin_cmd(pattern="(جلب الصورة|جلب الصوره|ذاتيه|ذاتية|حفظ)"))
...
@leaderthoniq.on(admin_cmd(pattern="(ذاتية تشغيل|الذاتية تشغيل|الذاتيه تشغيل|ذاتيه تشغيل)"))
...
@leaderthoniq.on(admin_cmd(pattern="(الذاتية تعطيل|الذاتيه تعطيل|ذاتية تعطيل|ذاتيه تعطيل)"))
