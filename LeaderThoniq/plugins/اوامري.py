...
LED_IC = "https://telegra.ph/file/46cc087ec231116cf18d1.jpg"
ROE = "**♰ هـذه هي قائمة اوامـر سـورس ليدرثون ♰**"
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if query.startswith("اوامري") and event.query.user_id == bot.uid:
            buttons = [
                [Button.inline("♰ اوامر الادمن ♰", data="leaderthoniq0")],
                [
            ...
            if LED_IC and LED_IC.endswith((".jpg", ".png", "gif", "mp4")):
                result = builder.photo(
                    LED_IC, text=ROE, buttons=buttons, link_preview=False
                )
            elif LED_IC:
...
@leaderthoniq.tgbot.on(CallbackQuery(data=re.compile(rb"leaderthoniq0")))
@check_owner
async def _(event):
    buttons = [
    [
...
@leaderthoniq.tgbot.on(CallbackQuery(data=re.compile(rb"jrzst")))
@check_owner
async def _(event):
    butze = [
    [
     Button.inline("التالي", data="tslrzj"),
     Button.inline("رجوع", data="leaderthoniq0")]]
    await event.edit(GRTSTI, buttons=butze)


@leaderthoniq.tgbot.on(CallbackQuery(data=re.compile(rb"tslrzj")))
@check_owner
async def _(event):
    buttons = [
    [
...
@leaderthoniq.tgbot.on(CallbackQuery(data=re.compile(rb"krrznd")))
@check_owner
async def _(event):
    buttons = [
    [
...
@leaderthoniq.tgbot.on(CallbackQuery(data=re.compile(rb"rozbot")))
@check_owner
async def _(event):
    buttons = [
    [
...
@leaderthoniq.tgbot.on(CallbackQuery(data=re.compile(rb"Jmrz")))
@check_owner
async def _(event):
    buttons = [
    [
...
@leaderthoniq.tgbot.on(CallbackQuery(data=re.compile(rb"r7brz")))
@check_owner
async def _(event):
    buttons = [
    [
...
@leaderthoniq.tgbot.on(CallbackQuery(data=re.compile(rb"sejrz")))
@check_owner
async def _(event):
    buttons = [
    [
...
@leaderthoniq.tgbot.on(CallbackQuery(data=re.compile(rb"gro")))
@check_owner
async def _(event):
    buttons = [
    [
...
@leaderthoniq.tgbot.on(CallbackQuery(data=re.compile(rb"grrz")))
@check_owner
async def _(event):
    buttons = [
    [
...
@leaderthoniq.tgbot.on(CallbackQuery(data=re.compile(rb"iiers")))
@check_owner
async def _(event):
    buttons = [
    [
...
@leaderthoniq.tgbot.on(CallbackQuery(data=re.compile(rb"rfhrz")))
@check_owner
async def _(event):
    buttons = [
    [
...
@leaderthoniq.tgbot.on(CallbackQuery(data=re.compile(rb"uscuxrz")))
@check_owner
async def _(event):
    buttons = [[Button.inline("رجوع", data="leaderthoniq0"),]]
