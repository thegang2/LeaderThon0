...
def pack_name(userid, pack, is_anim, is_video):
    if is_anim:
        return f"leaderthon_{userid}_{pack}_anim"
    elif is_video:
        return f"leaderthon_{userid}_{pack}_vid"
    return f"leaderthon_{userid}_{pack}"
...
async def kang(args):  # sourcery no-metrics
    ...
    if not user.username:
        try:
            ...
            username = f"leaderthon_{user.id}"
    else:
...
@leaderthoniq.on(admin_cmd(pattern="حزمة"))
async def leaderthonpkg(_):
    ...
    await edit_or_reply(
        _, f"**- تم اخذ الحزمه بنجاح ✓ \nالحزمه  → [اضغط هنا](https://t.me/addstickers/{leaderthon_Jep.set.short_name})**")
