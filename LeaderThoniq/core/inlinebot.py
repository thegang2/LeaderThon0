...
def main_menu():
    text = f"**- [سـورس ليدرثون](https://t.me/leaderthon)\
        \n المسـاعد\
        \n\
        \n◽ ليدرثون لـ {mention}**"
    buttons = [
...
@leaderthoniq.tgbot.on(InlineQuery)
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("leaderthoniq"):
            buttons = [
                (
                    Button.inline("الحـالة ⚒️", data="stats"),
                    Button.url(" leaderthon UsᴇʀBoᴛ", "https://t.me/leaderthon"),
                )
            ]
            ...
            builder = event.builder
            troll = os.path.join("./leaderthon", "troll.txt")
            try:
            ...
            result = builder.article(
                title="رسـالة تـرول من سـورس ليدرثون 🧸♥",
            ...
            user, txct = query.split(" ", 1)
            builder = event.builder
            secret = os.path.join("./leaderthon", "secrets.txt")
            try:
            ...
            result = builder.article(
                title="همسـة سـرية من سـورس ليدرثون 🧸♥",
                text=f"فقـط {razan} هـو مـن يستـطيع مشـاهدتهـا !",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            query = query[5:]
            builder = event.builder
            hide = os.path.join("./leaderthon", "hide.txt")
            try:
            ...
            result = builder.article(
                title="رسـالة مخفيـة من سـورس ليدرثون 🧸♥",
                text=f"✖✖✖",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            ...
            _result = main_menu()
            result = builder.article(
                title="© leaderthon Help",
                description="Help menu for leaderthon",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
        ...
        buttons = [
            (
                Button.url("قنـاة السـورس ⚒️", "https://t.me/leaderthon"),
                Button.url(
                    "كـروب المـساعدة 📬",
                    "https://t.me/leaderthon1",
                ),
            )
        ]
        markup = event.client.build_reply_markup(buttons)
        ...
        text, msg_entities = await event.client._parse_message_text(
            "**[بـوت ليدرثون 🧸♥](https://t.me/leaderthon)**\
            \n\
            \n❤ ليدرثون هـو بـوت بسـيط يدخـل الـى حسـابك لجعـلك تتحـكم به معـ اوامـر معينـة.\
            \n\
            \n**اذا كـنت تـرغب بتـنصيـب السـورس عـلى حسـابك ,\
            \n🐾 تعال الـى [كروب المسـاعدة](https://t.me/leaderthon1)!**",
            "md",
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
...
@leaderthoniq.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@leaderthoniq.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@leaderthoniq.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@leaderthoniq.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
