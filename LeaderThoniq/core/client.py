...
class LeaderClient(TelegramClient):
    def ar_cmd(
        self: TelegramClient,
        ...
        **kwargs,
        ...
        def decorator(func):  # sourcery no-metrics
                ...
                try:
                    ...
                    if not disable_errors:
                        ...
                        ftext = f"\nDisclaimer:\nThis file is pasted only here ONLY here,\
                                  ...
                                  \n\n--------BEGIN leaderthon TRACEBACK LOG--------\
                        ...
                        text = "**تقرير خطا ليدرثون**\n\n"
                        link = "[هنا](https://t.me/leaderthonSupport)"
            ...
            from .session import leaderthoniq
            ...
            if pattern is not None:
                ...
                if edited:
                    leaderthoniq.add_event_handler(
                        wrapper,
                ...
                if edited:
                    leaderthoniq.add_event_handler(func, events.MessageEdited(**kwargs))
                leaderthoniq.add_event_handler(func, events.NewMessage(**kwargs))
            return wrapper
    ...
    def bot_cmd(
        ...
        **kwargs,
        ...
        def decorator(func):
            async def wrapper(check):
                try:
                    ...
                    if not disable_errors:
                        ...
                        text = "**تقرير خطا ليدرثون**\n\n"
                        link = "[هنا](https://t.me/Groupleaderthon)"
                        text += "إذا كنت تريد يمكنك الإبلاغ عن ذلك"
                        text += f"- فقط قم بإعادة توجيه هذه الرسالة {link}.\n"
                        text += "لا يتم تسجيل اي خطا فقط التاريخ والوقت\n\n"
                        text += f"**⌯︙تقرير الخطأ : ** [{new['error']}]({pastelink})"
            ...
            if edited is True:
                leaderthoniq.tgbot.add_event_handler(func, events.MessageEdited(**kwargs))
            else:
                leaderthoniq.tgbot.add_event_handler(func, events.NewMessage(**kwargs))
...
LeaderClient.fast_download_file = download_file
LeaderClient.fast_upload_file = upload_file
LeaderClient.reload = restart_script
LeaderClient.get_msg_link = get_message_link
LeaderClient.check_testcases = checking
try:
    send_message_check = TelegramClient.send_message
except AttributeError:
    LeaderClient.send_message = send_message
    LeaderClient.send_file = send_file
    LeaderClient.edit_message = edit_message
